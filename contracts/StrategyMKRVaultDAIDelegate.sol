...
contract StrategyMKRVaultDAIDelegate {
  ...
  address public want; //// = address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
  ...
  function deposit() public {
    ...
    IeVault(yVaultDAI).depositAll();
  }

  function _getPrice() internal view returns (uint256 p) {
    (uint256 _read, ) = IOSMedianizer(eth_price_oracle).read();
    (uint256 _foresight, ) = IOSMedianizer(eth_price_oracle).foresight();
    p = _foresight < _read ? _foresight : _read;
  }

  function _checkDebtCeiling(uint256 _amt) internal view returns (bool) {
    (, , , uint256 _line, ) = IVatLike(vat).ilks(ilk);
  ...
  function _getDrawDart(address urn, uint256 wad) internal returns (int256 dart) {
    uint256 rate = IJugLike(jug).drip(ilk);
    uint256 _dai = IVatLike(vat).dai(urn);
  ...
  function _getWipeDart(uint256 _dai, address urn) internal view returns (int256 dart) {
    (, uint256 rate, , , ) = IVatLike(vat).ilks(ilk);
    (, uint256 art) = IVatLike(vat).urns(ilk, urn);
  ...
  function _withdrawAll() internal {
    IeVault(yVaultDAI).withdrawAll(); // get Dai
  ...
  function getUnderlyingDai() public view returns (uint256) {
    return IERC20(yVaultDAI).balanceOf(address(this)).mul(IeVault(yVaultDAI).getPricePerFullShare()).div(1e18);
  }

  function _withdrawDaiMost(uint256 _amount) internal returns (uint256) {
    uint256 _shares = _amount.mul(1e18).div(IeVault(yVaultDAI).getPricePerFullShare());
    ...
    IeVault(yVaultDAI).withdraw(_shares);
  ...
  function _withdrawDaiLeast(uint256 _amount) internal returns (uint256) {
    uint256 _shares =
      _amount.mul(1e18).div(IeVault(yVaultDAI).getPricePerFullShare()).mul(withdrawalMax).div(
  ...
  function _swap(uint256 _amountIn) internal {
    ...
    IUni(unirouter).swapExactTokensForTokens(_amountIn, 0, path, address(this), now.add(1 days));
  }
